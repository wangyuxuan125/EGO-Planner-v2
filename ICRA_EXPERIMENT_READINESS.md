...
- Schema v7 records goal/replan/attempt identity, timing, geometry, hard-parameterization coverage and junction violation, sampled curve penalty/violation, seed strategy, continuation candidates, monotonic acceptance and rollback reason.
...
2. Execute regression tests for at least 30 fixed seeds before large experiments. Confirm that every active hard junction has near-zero `max_junction_violation_final_m`, compare v7 success/latency against the v6 soft-only ablation, and verify no increase in final collision failures.
3. Freeze one CSV schema and aggregation script. Aggregate optimizer calls by independent `goal_id`; report replan-call success only as a secondary runtime metric. Add an FSM-level goal-arrival/timeout/collision record before calling the aggregate a mission-success rate: schema v7 identifies a goal but does not yet prove that the vehicle reached it.
